...
class USPSAddressPartner(models.Model):
    ...
    def button_usps_address_validation(self):
        view_ref = self.env.ref(
            "usps_address_validation.usps_address_validation_view_form"
        )
    ...
    def usps_xml_request(self):
        ...
        try:
            ...
            raise ValidationError(
                "Your credentials are not configured, please ensure you have a username, password, and API URL set under Contacts/Configuration/USPS Credential Configuration"
            )
        ...
        zip = str(self.zip)
        request = (
            '%s+?API=Verify&XML=<AddressValidateRequest USERID="%s"><Address ID="0"><Address1>%s</Address1><Address2>%s</Address2><City>%s</City><State>%s</State><Zip5>%s</Zip5><Zip4></Zip4></Address></AddressValidateRequest>'
            % (web, user_id, address1, address2, city, state, zip)
        )
        try:
            print("INPUT")
            print(request)
