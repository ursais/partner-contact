...
class ResConfigSettings(models.TransientModel):
    ...
    usps_api_url = fields.Char(string='API URL', default='https://secure.shippingapis.com/ShippingAPI.dll', config_parameter='usps_api_url')
    ...
    def get_values(self):
        ...
        res.update(
            usps_api_url = self.env['ir.config_parameter'].sudo().get_param('usps_api_url'),
            usps_username = self.env['ir.config_parameter'].sudo().get_param('usps_username'),
            usps_password = self.env['ir.config_parameter'].sudo().get_param('usps_password'),
        )
        return res
